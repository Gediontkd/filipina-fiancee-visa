...
class ProfileController extends Controller
{
    ...
    {
        if ($page == 'progress') {
            $incomplete = UserFianceVisaType::where('user_id', Auth::id())->where('status', 'in-progress')->first();
            if ($incomplete) {
                if ($incomplete->type == 'alien') {
                    return redirect()->route('fianceAlienApplication');
                } elseif ($incomplete->type == 'sponsor') {
                    return redirect()->route('fianceSponsorApplication');
                } elseif ($incomplete->type == 'alien-children') {
                    return redirect()->route('fianceAlienChildApplication');
                }
            }
        }
        $user = Auth::user();
        ...
        switch ($submission->application_id) {
            case 1: // Fiancée Visa (OLD SYSTEM)
                $alienCount = FianceVisaSubmittedStep::where('user_id', Auth::id())
                    ->where('type', 'alien')->distinct('step')->count('step');
                $sponsorCount = FianceVisaSubmittedStep::where('user_id', Auth::id())
                    ->where('type', 'sponsor')->distinct('step')->count('step');
                $alienChildrenCount = FianceVisaSubmittedStep::where('user_id', Auth::id())
                    ->where('type', 'alien-children')->distinct('step')->count('step');
