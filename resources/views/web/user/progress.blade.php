                ...
                @elseif($activeSubmission)
                    ...
                    @php
                        ...
                        if ($applicationType == 1) {
                            // Fiancée Visa (K-1) - Use same calculation as profile method
                            $sponsorCount = \App\Models\FianceVisaSubmittedStep::where('user_id', Auth::id())->where('type', 'sponsor')->distinct('step')->count('step');
                            $alienCount = \App\Models\FianceVisaSubmittedStep::where('user_id', Auth::id())->where('type', 'alien')->distinct('step')->count('step');
                            $childrenCount = \App\Models\FianceVisaSubmittedStep::where('user_id', Auth::id())->where('type', 'alien-children')->distinct('step')->count('step');

                            $overAll = (($sponsorCount + $alienCount + $childrenCount) / 36) * 100;
